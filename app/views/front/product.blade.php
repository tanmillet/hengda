        ...
        <ul>
            @foreach($programa as $key=>$pro)
                <li class="{{(($key + 1) == count($programa)) ? 'mar' : ''}}"><a href="{{$pro['url']}}"
                                                                                 style="{{ isUrl($pro['url'] , $lang) ? 'border-bottom: solid 2px #599bf8; color: #599bf8;' : '' }}">{{$pro['name']}}</a>
                </li>
            @endforeach
                <li class="link"><a class="{{isset($lang) && ($lang == 'zh') ? 'focu' : ''}}" href="{{getRouteDeUrl()}}/index/zh">中文版</a></li>
                <li class="link"><a class="{{isset($lang) && ($lang == 'en') ? 'focu' : ''}}" href="{{getRouteDeUrl()}}/index/en">English</a></li>
        </ul>
    </div>
</div>
...
<div class="page02 main">
    <div class="le" style="margin-bottom: 100px;">
        <ul>
            @forelse($productTypes as $productType)
                <li><a style="font-family: 微软雅黑;" class="{{($productType->id == $selProductTypeId)? 'focu' : ''}}"
                       href="{{getRouteDeUrl()}}/product/{{$lang}}/{{$productType->id}}">
        ...
        <div class="searchBox01" style="padding-top: 40px;">
            <form action="{{getRouteDeUrl()}}/product/{{$lang}}" method="post">
                <input type="text" style="font-size: 12px;height: 30px;"
                       placeholder="{{(isset($lang) && $lang == 'zh') ? '产品 查询 ...' : 'product search ...' }}"
                       name="selProductName" value="">
    ...
    <div class="ri mar"
         style="padding: 50px 100px 25px !important; width: 864px!important;line-height: 26px!important;" id="page_cx">

        <?php echo htmlspecialchars_decode($desc)?>

        <div class="product" style="display: block;margin-top: 40px;">
            <ul class="ul01">
                @forelse($productLists as $productList)
                    <li style="margin-left: 0px!important;margin-right: 74px;">
                        <h2><a href="{{getRouteDeUrl()}}/productinfo/{{$lang}}/{{$productList->id}}"><img
                                        src="{{$productList->product_img}}"></a></h2>
